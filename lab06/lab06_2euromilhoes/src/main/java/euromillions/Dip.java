...
public class Dip {
    ...
    private static final int STAR_MAX = 11;
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        for (int s: arrayOfStarts) {
            if (s > STAR_MAX) {
                throw new IllegalStateException("invalid star value (1-12)");
            }
        }
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < NUMS; i++ ) {
            int candidate =new SecureRandom().nextInt(NUM_MAX) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
            }
        }
        for (int i = 0; i < STARS; i++ ) {
            int candidate =new SecureRandom().nextInt(STAR_MAX) + 1;
